...
class GaussianSampler(BaseSampler):
    ...
    def sample_action(self, actions):
        action_var = torch.full(actions.shape[1:], self.std * self.std, device=self.args.device)
    ...
    def get_variances(self, actions):
        return torch.full(actions.shape[1:], self.std * self.std, device=self.args.device)
